...
def buildTriangles( slice0, slice1 ):
    ...
    minDist, minIndices = math.inf, (0, 0)

    for i, verts0 in enumerate(point1):
        for j, verts1 in enumerate(point2):
            dist = length(subtract(verts0.coords, verts1.coords))
            if dist < minDist:
                minDist = dist
                minIndices = (i, j)
    ...
    cyclicPerm0 = point1[perm0:] + point1[:perm0 + 1]
    cyclicPerm1 = point2[perm1:] + point2[:perm1 + 1]
    ...
    minArea = [[0 for r in range(len(point1) + 1)] for r in range(len(point2) + 1)]
    minDir = [[0 for r in range(len(point1) + 1)] for r in range(len(point2) + 1)]
    ...
    # [YOUR CODE HERE]
    for r1 in range(1, len(cyclicPerm0)):
        minArea[0][r1] = triangleArea(cyclicPerm0[r1-1].coords, cyclicPerm0[r1].coords, cyclicPerm1[0].coords) + minArea[0][r1-1]
        minDir[0][r1] = Dir.PREV_COL
    ...
    # [YOUR CODE HERE]
    for r2 in range(1, len(cyclicPerm1)):
        minArea[r2][0] = triangleArea(cyclicPerm1[r2 - 1].coords, cyclicPerm1[r2].coords, cyclicPerm1[0].coords) + minArea[r2 - 1][0]
        minDir[r2][0] = Dir.PREV_ROW
    ...
    # [YOUR CODE HERE]
    for r3 in range(1, len(cyclicPerm1)):
        for r4 in range(1, len(cyclicPerm0)):
            leftArea = triangleArea(cyclicPerm0[r4 - 1].coords, cyclicPerm0[r4].coords, cyclicPerm1[r3].coords) + minArea[r3][r4 - 1]
            upperArea = triangleArea(cyclicPerm1[r3 - 1].coords, cyclicPerm0[r4].coords, cyclicPerm1[r3].coords) + minArea[r3 - 1][r4]

            if (leftArea <= upperArea):
                minArea[r3][r4], minDir[r3][r4] = leftArea, Dir.PREV_COL
            else:
                minArea[r3][r4], minDir[r3][r4] = upperArea, Dir.PREV_ROW
    ...
    while running == 1:
        if minDir[r][c] == Dir.PREV_ROW:
            triangles.append(Triangle([cyclicPerm1[r], cyclicPerm0[c], cyclicPerm1[r - 1]]))
            r -= 1
        else:
            triangles.append(Triangle([cyclicPerm1[r], cyclicPerm0[c], cyclicPerm0[c - 1]]))
            c -= 1
...
def main():
    ...
    print( 'Press "/" to see key controls' )
