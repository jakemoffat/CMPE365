...
def buildTriangles(slice0, slice1):
    ...
    for a in range(1, len0plus1 + 1):
        minArea[0][a] = triangleArea(newsliceVerts0[a - 1].coords, newsliceVerts1[0].coords, newsliceVerts0[a].coords) + \
                        minArea[0][a - 1]
    ...
    for b in range(1, len1plus1 + 1):
        minArea[b][0] = triangleArea(newsliceVerts1[b - 1].coords, newsliceVerts0[0].coords, newsliceVerts1[b].coords) + \
                        minArea[b - 1][0]
    ...
    for c in range(1, len1plus1 + 1):
        ...
        for d in range(1, len0plus1 + 1):
            minArea[c][d] = min((minArea[c][d - 1] + triangleArea(newsliceVerts0[d - 1].coords,
                                                                  newsliceVerts0[d].coords, newsliceVerts1[c].coords)),
                                (minArea[c - 1][d] + triangleArea(newsliceVerts1[c - 1].coords,
                                                                  newsliceVerts0[d].coords, newsliceVerts1[c].coords)))

            # Determine direction is either a prev_col or prev_row
            if minArea[c][d] == minArea[c][d - 1] + triangleArea(newsliceVerts0[d - 1].coords, newsliceVerts0[d].coords,
                                                                 newsliceVerts1[c].coords):
    ...
    while run == 1:
        if minDir[slice1Len][slice0Len] == Dir.PREV_COL:
            triangles.append(
                Triangle([newsliceVerts1[slice1Len], newsliceVerts0[slice0Len], newsliceVerts0[slice0Len - 1]]))
            slice0Len -= 1
        else:
            triangles.append(
                Triangle([newsliceVerts1[slice1Len], newsliceVerts0[slice0Len], newsliceVerts1[slice1Len - 1]]))
            slice1Len -= 1
