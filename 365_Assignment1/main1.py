...
def buildHull(points):
    ...
    if full_length <= 3:
        ...
        buildHull(points[:half_length])  # Front End
        buildHull(points[half_length:])  # Back End

        UpRight = points[half_length - 1]
        UpLeft = points[half_length]

        DownRight = points[half_length - 1]
        DownLeft = points[half_length]

        while ((turn(UpLeft.ccwPoint, UpLeft, UpRight) == 1) or (turn(UpLeft, UpRight, UpRight.cwPoint) == 1)):  # Left Turn
            if (turn(UpLeft.ccwPoint, UpLeft, UpRight) == 1):

                UpLeft = UpLeft.ccwPoint
                UpLeft.cwPoint.ccwPoint = None

                if full_length > 4:
                    UpLeft.cwPoint = None

            else:
                UpRight = UpRight.cwPoint
                UpRight.ccwPoint.cwPoint = None

                if full_length > 4:
                    UpRight.ccwPoint = None

            # Repeat while and if loops but for downwards walking

        #Removing of points from pre-existing hulls
        while ((turn(DownLeft.cwPoint, DownLeft, DownRight) == 2) or (
                turn(DownLeft, DownRight, DownRight.ccwPoint) == 2)):  # Right Turn
            if (turn(DownLeft.cwPoint, DownLeft, DownRight) == 2):
                DownLeft = DownLeft.cwPoint
                DownLeft.ccwPoint.cwPoint = None
                DownLeft.ccwPoint = None
            else:
                DownRight = DownRight.ccwPoint
                DownRight.cwPoint.ccwPoint = None
                DownRight.cwPoint = None

        DownLeft.ccwPoint = DownRight  # Final point declaration
        DownRight.cwPoint = DownLeft
        UpRight.ccwPoint = UpLeft
        UpLeft.cwPoint = UpRight
