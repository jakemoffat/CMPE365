...
def buildHull( points ):
    ...
    #
    # [YOUR CODE HERE]
    fullLength = (len(points))
    halfLength = fullLength//2

    if fullLength <= 3:
        if fullLength != 2: # base case
        ...
        buildHull(points[:halfLength]) #Front
        buildHull(points[halfLength:]) #Back

        topLeft = points[halfLength]
        topRight = points[halfLength - 1]
        bottomLeft = points[halfLength]
        bottomRight = points[halfLength - 1]
        ...
        while ((turn(topLeft.ccwPoint, topLeft, topRight) == 1) or (turn(topLeft, topRight, topRight.cwPoint) == 1)):
            if (turn(topLeft.ccwPoint, topLeft, topRight) == 1):
                topLeft = topLeft.ccwPoint
                topLeft.cwPoint.ccwPoint = None
                if fullLength > 4:
                    topLeft.cwPoint = None
            else:
                topRight = topRight.cwPoint
                topRight.ccwPoint.cwPoint = None
                if fullLength > 4:
                    topRight.ccwPoint = None

        # Removing of points from pre-existing hulls
        while ((turn(bottomLeft.cwPoint, bottomLeft, bottomRight) == 2) or (turn(bottomLeft, bottomRight, bottomRight.ccwPoint) == 2)):  # Right Turn
            if (turn(bottomLeft.cwPoint, bottomLeft, bottomRight) == 2):
                bottomLeft = bottomLeft.cwPoint
                bottomLeft.ccwPoint.cwPoint = None
                bottomLeft.ccwPoint = None
            else:
                bottomRight = bottomRight.ccwPoint
                bottomRight.cwPoint.ccwPoint = None
                bottomRight.cwPoint = None

        topLeft.cwPoint = topRight  # Final point declaration
        topRight.ccwPoint = topLeft

        bottomLeft.ccwPoint = bottomRight  # Final point declaration
