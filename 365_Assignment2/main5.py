...
def buildTristrips(triangles):
    ...
    while i != len(triangles) - 1:  # If we haven't checked all elements of triangles, we keep making strips,
        ...
        i = 0
        start = triangles[
            i]  # Just start at first triangle, a triangles adjacent triangles can change every iteration, so we just start at the beggining every time
        op2 = None  # Second option variable

        while (start.openNumber() != 1 or start.nextTri != None) and i < len(
                triangles) - 1:  # O(n), will usually run n times, but won't if a corner is found
            # While  "not a corner"    or  "open triangle"       and  "in range of triangles"

            if start.openNumber() == 2 and op2 == None:  # If we happen to find a triangle thats an edge, I keep track of it
                op2 = start
        ...
        if start.openNumber() != 1 and op2 != None:  # If we made it though the first loop and we didn't find a corner but we did find an edge, use the edge
            start = op2
        ...
        if i != len(triangles) - 1:
            ...
            while current.openNumber() != 0:  # while we don't have a triangle with no open triangles (full). O(n), could theoretically run n times, but that would create a 1 perfect strip
                x = current.leastFullTri()  # Find best adjacent triangle
    ...
    for tri in triangles:  # O(n)
        if tri.openNumber() == 0 and tri.nextTri == None and tri.prevTri == None:  # Checking if there are any stips of length 1
            count += 1
