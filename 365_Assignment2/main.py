...
# ================================================================
# ================================================================
# Functions before buildTristrips
# First function: Needs to find the triangle that has a minimum
# number of adjacent non-strip triangles and select it
def minNonstripTri(triangle):
    # records number
    adjNonstrip = [5] * len(triangle)
    for i, tri in enumerate(triangle):
        if tri.nextTri == None and tri.prevTri == None:
            counter = 0
            for adjTriangle in tri.adjTris:
                if adjTriangle.nextTri == None and adjTriangle.prevTri == None:
                    counter += 1
            adjNonstrip[i] = counter
    # now find the minimum
    minNonadj = min(adjNonstrip)
    i1 = -1 # initial index
    if minNonadj == 5:
        return i1
    # min index and if its not in the skip index
    for i in range(len(triangle)):
        if adjNonstrip[i] == minNonadj:
            i1 = i
            break
    return i1

# Second function
def adjacentTri(triangle):
    adjTri = None
    minAdjTri = 4
    for tri in triangle.adjTris:
        if tri.nextTri == None and tri.prevTri == None:
            counter = 0
            for adj_triangle in tri.adjTris:
                if adj_triangle.nextTri == None and adj_triangle.prevTri == None:
                    counter += 1
            if counter < minAdjTri:
                minAdjTri = counter
                adjTri = tri
    return adjTri
...
def buildTristrips(triangles):
    ...
    # [YOUR CODE HERE]
    i = minNonstripTri(triangles)

    while i != -1:
        startTri = triangles[i]

        # Increment 'count' every time you *start* a new triStrip.
        # The starting triangle should be the current triangle
        currTri = startTri
        while True:
            nextTri = adjacentTri(currTri)
            if nextTri == None:
                count += 1
                if currTri == startTri:
                    currTri.prevTri = currTri
                break

            nextTri.prevTri = currTri
            currTri.nextTri = nextTri
            currTri = nextTri
